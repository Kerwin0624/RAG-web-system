...
class IngestService:
    ...
    def _validate_path(self, path: Path) -> Path:
        ...
        if self._settings.app_env.lower() != "prod":
            return resolved
        allowed_roots = [self._settings.data_dir.resolve(), Path(tempfile.gettempdir()).resolve()]
        if any(self._is_subpath(resolved, root) for root in allowed_roots):
            return resolved
        raise IngestError(f"生产环境仅允许读取 data 目录或系统临时目录下文件: {path}")
    ...
    def ingest(
        ...
        if directory:
            directory_path = self._validate_path(Path(directory))
    ...
    def _build_splitter(
        ...
        size = chunk_size if chunk_size is not None else self._settings.chunk_size
        overlap = chunk_overlap if chunk_overlap is not None else self._settings.chunk_overlap
